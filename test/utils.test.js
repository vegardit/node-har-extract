...
test('`makeBufferFromContent()` returns Buffer instance from base64 encoded content', t => {
  ...
  const content = {
    text: Buffer.from(plainText, 'utf8').toString(encoding),
    encoding,
  };
  const buf = utils.makeBufferFromContent({ content });
  ...
  t.is(
    buf.toString('utf8'),
    plainText,
    'should decode to original input value',
  );
});
...
test.todo(
  '`splitResponseBodyFromEntry()` creates buffer with response content for each entry',
);
...
test.todo(
  '`cloneHARAndReplaceEntries()` clones har object and puts replaces entries in clone',
);

test.todo(
  '`writeHARToFile()` stringifies har object and writes the contents to file',
);
