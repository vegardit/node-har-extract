...
test('`addFilenameToEntries()` adds `filename` prop to each entry', (t) => {
  ...
  const items = Array(entriesCount).fill('a').map(() => ({ entry: { foo: 'bar' } }));
...
test.todo('`writeEntryResponseBodiesToTargetDir()` writes contents of each entry\'s buffer to `targetDir`');
...
test.todo('`splitResponseBodyFromEntries()` invokes `splitResponseBodyFromEntry()` for each log.entry.response');

test.todo('`replaceResponseContentTextWithFilename()` clones entry and puts `filename` into clone.response.content.text');

test.todo('`replaceResponseContentTextWithFilenames()` incokes `replaceResponseContentTextWithFilename()` for each item');
