...
const readHARFromFile = ({
  file, encoding = 'utf8', flag = 'r', validate = false,
...
const splitResponseBodyFromEntry = ({ entry }) => harValidator.content(_.get(entry, 'response.content')).then((content) => {
  if (!content.text) {
    return { entry };
  }
